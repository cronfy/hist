...
## Настроить внешний вид шаблона

Похоже, пора читать [Quick start guide](https://mmistakes.github.io/minimal-mistakes/docs/quick-start-guide/).

* Там написано в [Starting Fresh](https://mmistakes.github.io/minimal-mistakes/docs/quick-start-guide/#starting-fresh), что раз мы начинали с пустой папки, нужно воссоздать _config.yml. Ок, заменяем наш _config.yml на их, но только нужно вернуть наши параметры. К этому моменту мы уже меняли:
    * baseurl - надо прописать заново.
    * plugins - то, что мы прописывали, там уже есть, можно не трогать.
    * theme - в примере закомментировано, раскомментируем.

После этого желательно проверить, что jekyll нормально работает.

```
jekyll serve
```

Еще важный момент - перевод сообщений на русский. В разделе [UI Text](https://mmistakes.github.io/minimal-mistakes/docs/ui-text/), есть ссылка на файл `ui-text.yml`, берем его и кладем в `_data/ui-text.yml`. В `locale` в конфиге нужно прописать `ru-RU` и переапустить serve. Надписи шаблона должны стать русскими.

И теперь начинается самое сложное - нужно придумать название для блога. Испытав муки творчества, записываем результат в `title`. Но заранее предупреждаю, что страдания на этом не заканчиваются - нужно еще придумать description, bio, загрузить logo, teaser, avatar, в общем, развлечений много.

Все это можно сделать, последовательно выполняя описанные в [Configuration](https://mmistakes.github.io/minimal-mistakes/docs/configuration/) рекомендации.

(...три часа спустя...)

 * Где показывается teaser, так и не понял. Есть предположение, что он заработает, когда появится блок Related posts, а для этого нужно написать второй пост. Отложим на потом.
 * На logo меня не хватило. Может, потом вдохновение придет.
 * Пока перерыв, продолжим с Breadcrumb navigation

Вдоволь наигравшись с конфигом
